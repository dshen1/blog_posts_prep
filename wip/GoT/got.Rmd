...
predictions[grep("Aegon", predictions$name), c(6, 12:13, 15, 34)] %>% arrange(name)
```

http://awoiaf.westeros.org/index.php/Rodwell_Stark

```{r}
predictions[predictions$name == "Lyanna Stark", "spouse"] <- "Rhaegar Targaryen"
...
predictions[grep("Rhaegar Targaryen", predictions$name), c(6, 15:16)] %>% arrange(name)
```
...
net_Stark$lty <- ifelse(net_Stark$source == "Lyanna Stark" & net_Stark$target == "Rhaegar Targaryen", "dotted", net_Stark$lty)
...
characters_Targaryen <- characters %>%
  ...
  filter(name %in% c("Daenerys Targaryen", "Rhaegar Targaryen", "Viserys Targaryen"))
...
net_Targaryen$lty <- ifelse(net_Targaryen$source == "Lyanna Stark" & net_Targaryen$target == "Rhaegar Targaryen", "dotted", net_Targaryen$lty)
