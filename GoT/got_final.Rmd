...
How do we find out who the most important character(s) is/are in this network? We consider a character "important" if he has connections to many other characters.

There are a few network properties, that tell us more about this. For most of them, I am considering the network as undirected to account for parent/child relationships as being mutual.
...
betweenness_std <- betweenness / ((vcount(union_graph_undir) - 1) * (vcount(union_graph_undir) - 2))
...
PageRank (originally from Google) is similar to eigenvector centrality.
...
We can also calculate the eigenvalues and eigenvectors of the adjacency matrix.
...
```{r}
union_characters %>%
  ggplot(aes(x = popularity, color = house2, fill = house2)) +
...
The largest group of nodes is:

```{r fig.width=20, fig.height=20}
vcol <- rep("grey80", vcount(union_graph_undir))
vcol[unlist(largest_cliques(union_graph_undir))] <- "red"
