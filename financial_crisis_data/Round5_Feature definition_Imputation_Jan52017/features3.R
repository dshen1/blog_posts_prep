...
###########################################################################
###########################################################################
# IMPUTATION FOR THE VARIABLES HPNOM, XRUSD, STOCKS, TLOANS and GDP using interpolation
# for (Features 4, 5 and 3), flanks omitted

#interpolation

interp.no.flanks <- function(x){ #this function omits flanks in interpolation
...
for(i in levels(factor(completeData2$iso))){  #impute data for each iso. dplyr group_by giving problems
...
write.csv(crises_jst2,"crises_with_features.csv")
